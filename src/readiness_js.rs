...
//!
//! There is deliberately no "the app never mounted" verdict. One was tried —
//! no LCP candidate, no structural mutation, almost no text at 3 s — and it
//! could not tell a dead SPA shell from an ordinary small static page, which
//! has exactly the same signature. It reported `shell` for a plain HTML
//! fixture. Any threshold tight enough to avoid that (an all-but-empty body)
//! misses the real shells, which ship spinners and noscript blocks. Telling an
//! agent a working page is dead is worse than the problem it aimed at.
//!
